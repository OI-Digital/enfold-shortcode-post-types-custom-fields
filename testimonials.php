...
if ( ! class_exists( 'avia_sc_wd_testimonial' ) )
{
	class avia_sc_wd_testimonial extends aviaShortcodeTemplate
	{
		/**
		 * @since < 4.0
		 * @var string 
		...
		function shortcode_insert_button()
		{
			...
			$this->config['name']			= __( 'WD Testimonials Slider', 'avia_framework' );
			$this->config['tab']			= __( 'Content Elements', 'avia_framework' );
			$this->config['icon']			= AviaBuilder::$path['imagesURL'] . 'sc-testimonials.png';
			$this->config['order']			= 20;
			$this->config['target']			= 'avia-target-insert';
			$this->config['shortcode']		= 'av_wdtestimonials';
			//$this->config['shortcode_nested'] = array( 'av_wd_testimonial_single' );
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			// Query our testimonial CPT & ACF fields
			$args = array(
			...
			while ( $query1->have_posts() ) {
				...
				$markup_person = get_field('name', get_the_ID());
				$markup_content = get_the_content(get_the_ID());
				
				if ( $testi_counter == 1 ) {
					$testi_class = 'avia-first-testimonial';
				} elseif ( $testi_counter == 6 ) {
					$testi_class = 'avia-last-testimonial';
				} else {
					$testi_class = '';
				}
				
				$output .= "      <div class='avia-testimonial av_one_half flex_column no_margin avia-testimonial-row-1 avia-first-testimonial' >
						 <div class='avia-testimonial_inner'  itemscope=\"itemscope\" itemtype=\"https://schema.org/CreativeWork\" >
							<div class='avia-testimonial-content '  >
							   <div class='avia-testimonial-markup-entry-content'  itemprop=\"text\" >" . wpautop($markup_content, false) . "</div>
							</div>
							<div class='avia-testimonial-meta'>
							   ...
							   <div class='avia-testimonial-meta-mini'  itemprop=\"author\" itemscope=\"itemscope\" itemtype=\"https://schema.org/Person\" ><strong  class='avia-testimonial-name'    itemprop=\"name\" >" . $markup_person . "</strong></div>
							</div>
						 </div>
					  </div>";
